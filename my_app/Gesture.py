    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands = 2,min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                image = cv2.flip(image, 1)  # Flip first to avoid processing extra pixels
                image = cv2.resize(image, (640, 480))  # Reduce image size before conversion
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert after resizing
                image.flags.writeable = False  # Disable writing to reduce memory usage

                results = hands.process(image)  # Process the optimized image
                ...
                #             cv2.line(image, pv, (i[1],i[0]), (0, 0, 255), 2)
